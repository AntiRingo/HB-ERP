...
// console.log(generateCode(3,2225,list))
		...
		axios({
			method:"post",
			url:"product/selectAbandonedProduct?currentPage="+1+"",
		...
		}).then(function (resp)
		{
			...
			if (data.length>0){
				...
				function page(pages){
					...
					axios({
						method:"post",
						url:"product/selectAbandonedProduct?currentPage="+pages+"",
					...
					}).then(function (resp)
					{
						...
						for (let i = 0; i < updateP.length; i++) 	{
							updateP[i].onclick=function (){
								...
								}).then(function (resp)
								{
									...
									axios({
										method:"post",
										url:"attribute/selectAbandonedAttribute",
										data:data[i].id
									}).then(function (resp){
						...
						for (let i = 0; i < deleteP.length; i++) {
							deleteP[i].onclick=function (){
								...
								promise.then(function (resp){
									for (let j = 0; j < resp.length; j++) {
										if (resp[j].function_two_id===44 && resp[j].open_status===1){
											promiseBoolean=true;
											break;
										}
									}
